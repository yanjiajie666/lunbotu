...
//1.全局域 :从页面被打开之后到页面关闭之前始终是存在的
//2.函数域:存在函数被调用的一瞬间(也不一定,要考虑闭包的存在)
...
//1.在插件中,最好不要使用id,原因插件是为了能够被重复使用,也就是说在一个页面上可能会重复调用,会造成页面的冲突,并且具有唯一性
...
//4.插件的文件命名的问题:index.js,index.css ,命名太大众化,比如 jQuery.Slide.js